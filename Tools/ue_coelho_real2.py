...
# delete the bad anim-only asset
if EAL.does_asset_exist("/Game/Alice/Characters/CoelhoReal/SK_CoelhoReal"):
    EAL.delete_asset("/Game/Alice/Characters/CoelhoReal/SK_CoelhoReal")

t = unreal.AssetImportTask()
t.set_editor_property("filename", r"E:\References\3D\SK_Coelho.fbx")
...
print("SK_CoelhoReal class", sk.get_class().get_name() if sk else None)

# material from already-imported coelho textures
base=unreal.load_asset("/Game/Alice/Textures/Coelho/T_Coelho_Base")
norm=unreal.load_asset("/Game/Alice/Textures/Coelho/T_Coelho_Normal")
mr=unreal.load_asset("/Game/Alice/Textures/Coelho/T_Coelho_MR")
...
def tx(tt,stype,x,y):
    ...
    if tt: n.set_editor_property("texture", tt)
...
if sk and M and sk.get_class().get_name()=="SkeletalMesh":
    ...
    print("applied M_CoelhoReal", len(mats), "h~", round(sk.get_bounds().box_extent.z*2,1))
print("COELHO_REAL2_DONE")
