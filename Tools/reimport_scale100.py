"""Reimporta corpo + vestido com import_uniform_scale=100 (m->cm).
Mixamo exporta em metros, UE precisa cm."""
import unreal
L = lambda s: unreal.log(f"[S100] {s}")
tools=unreal.AssetToolsHelpers.get_asset_tools()

# limpa
for d in ("/Game/Alice/Characters/AliceBody","/Game/Alice/Characters/AliceDress2"):
...
sid=ob.skeletal_mesh_import_data
sid.set_editor_property("import_uniform_scale", 100.0)
ob.skeletal_mesh_import_data=sid
tb.options=ob
...
try:
    bb=body.get_bounds(); L(f"body bounds={bb.box_extent}")
except: pass

# VESTIDO scale 100 no mesmo skel
DSTD="/Game/Alice/Characters/AliceDress2"
td=unreal.AssetImportTask(); td.filename=r"E:\References\3D\alice_vestido_FINAL.fbx"
...
sid2=od.skeletal_mesh_import_data
sid2.set_editor_property("import_uniform_scale", 100.0)
od.skeletal_mesh_import_data=sid2
td.options=od
tools.import_asset_tasks([td])
dress=unreal.load_asset(f"{DSTD}/SK_AliceDress")
try:
    db=dress.get_bounds(); L(f"dress bounds={db.box_extent}")
except: pass

# anims tb (so re-skel — anims ja existem mas referenciam old skel; re-importa)
import os
DSTA="/Game/Alice/AnimAlice"
for a in unreal.EditorAssetLibrary.list_assets(DSTA, recursive=True):
    unreal.EditorAssetLibrary.delete_asset(a)
...
for src,dst in clips:
    ...
    o.anim_sequence_import_data.set_editor_property("import_uniform_scale", 100.0)
    t.options=o
